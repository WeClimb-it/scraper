...
class DownloadContent extends HttpOperation {
  //Responsible for downloading files and images from a given page.
  ...
  validateOperationArguments() {
    if (!this.scraper.config.filePath && !this.config.filePath)
      throw new Error(
        `DownloadContent operation Must be provided with a filePath, either locally or globally.`
      );
    if (!this.querySelector || typeof this.querySelector !== "string")
      throw new Error(
        `DownloadContent operation must be provided with a querySelector.`
      );
  }
  ...
  async scrape({ html, url }) {
    if (!this.directoryVerified) {
      await verifyDirectoryExists(
        this.config.filePath || this.scraper.config.filePath
      );
  ...
  getFileRefs(url, elementList, baseUrlFromBaseTag) {
    ...
    elementList.forEach((element) => {
      ...
      if (!src || isImageDataUrl) {
        //If the element doesn't have an "src" tag(in case of content type image), or the src is base64, try getting an alternative.
        const alternativeAttrib =
          this.alternativeSrc && this.getAlternativeAttrib(element[0].attribs);
  ...
  getAlternativeAttrib(alternativeAttribs) {
    for (let attrib in alternativeAttribs) {
      if (typeof this.alternativeSrc === "object") {
        ...
        if (this.alternativeSrc === attrib) return attrib;
      }
    }
  }
  ...
  saveDataUrlPromiseFactory(url) {
    return async () => {
      this.scraper.log(
        "Src is base64. Creating a file form it, with a hashed name."
      );
      ...
      const fileProcessor = new FileProcessor({
        ...
        path: this.config.filePath || this.scraper.config.filePath,
      });
      ...
      await writeFile(
        `${this.config.filePath || this.scraper.config.filePath}/${fileName}`,
        base64Data,
        "base64"
      );
  ...
  async getFile(url) {
    if (url.startsWith("data:image")) {
      ...
      const options = {
        ...
        shouldBufferResponse:
          this.config.contentType === "image" ? true : false,
    ...
    return await this.qyuFactory(() =>
      this.repeatPromiseUntilResolved(promiseFactory, url)
    );
  }
