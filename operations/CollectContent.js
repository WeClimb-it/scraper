...
class CollectContent extends Operation {
  ...
  validateOperationArguments() {
    if (!this.querySelector || typeof this.querySelector !== "string")
      throw new Error(
        `CollectContent operation must be provided with a querySelector.`
      );
  }
  ...
  async scrape({ html, url }) {
    ...
    for (let element of elementList) {
      let content = getNodeContent(element, {
        shouldTrim: this.config.shouldTrim,
        contentType: this.config.contentType,
      });

      if (this.config.getElementContent) {
        const contentFromCallback = await this.config.getElementContent(
          content,
          parentAddress,
          element
        );

        if (typeof contentFromCallback === "string") {
          content = contentFromCallback;
        }
      }
