...
class Request {
  constructor(config) {
    ...
    const defaultConfig = {
      ...
      headers: null,
      proxy: null,
      responseType: "text", //'text','json' or 'stream'. If 'stream' is chosen, the stream itself is returned. Otherwise, the FINAL output of the request is returned.
      auth: null,
    };
    ...
    if (config.headers) {
      const isEmpty =
        ...
        config.headers.constructor === Object;
  ...
  handleStatusCodes(customResponse) {
    if (customResponse.status >= 400) {
      const error = new CustomError({
        code: customResponse.status,
        response: customResponse,
        message: `Server responded with ${customResponse.status}`,
      });
      throw error;
    }
  }

  createCustomErrorFromFetchError(fetchError) {
    //Fetch errors are thrown only for network errors. There is no actual "response".
